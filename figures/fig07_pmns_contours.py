#!/usr/bin/env python3
# Generates: Figure 7 in manuscript section 14
# PMNS mixing angles: UDT predictions vs NuFIT experimental bands
"""
fig07 -- PMNS mixing angle comparison.

Loads: lib/angular_integrals.py (pmns_mixing_angles)
Three panels: sin^2(theta_12), sin^2(theta_23), sin^2(theta_13)
...
# Panel data: (key, xlabel, fraction_label)
panels = [
...
# NuFIT 5.2 Normal Ordering (central, 1sigma, 2sigma, 3sigma)
nufit_data = {
    'sin2_12': {
        'central': 0.304,
        'sigma': 0.012,
    ...
    'sin2_13': {
        'central': 0.02219,
        'sigma': 0.00062,
        '1sig': (0.02157, 0.02281),
        '2sig': (0.02096, 0.02344),
        '3sig': (0.02034, 0.02404),
    },
}
...
sigma_colors = ['#4CAF50', '#FFC107', '#FF9800']  # 1sig, 2sig, 3sig
sigma_alphas = [0.4, 0.25, 0.15]
sigma_labels = [r'$1\sigma$', r'$2\sigma$', r'$3\sigma$']

for ax, (key, xlabel, frac) in zip(axes, panels):
    pred = pmns[key]['predicted']
    nf = nufit_data[key]
    ...
    # NuFIT central value
    ax.axvline(nf['central'], color='0.4', linewidth=0.8, linestyle=':',
               zorder=2)
    ...
    # Fraction label
    tension = pmns[key]['tension_sigma']
    ...
    # Set x-range to show full 3-sigma band with padding
    lo3, hi3 = nf['3sig']
    pad = (hi3 - lo3) * 0.3

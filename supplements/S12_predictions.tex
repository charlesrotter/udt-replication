...
$\bar\rho = 1/(2\pi) = 0.1592$ ($0.02\sigma$).
$\bar\eta = 9/(8\pi) = 0.3581$ ($1.0\sigma$).
...
$v = m_\pi \times m_p / m_e = 504\pi^6 m_e = 247.6$~GeV ($+0.6\%$).
Exact algebraic identity of angular mass formulas (DERIVED).
$\lambda = \pi/\Sigma(2j_\kappa+1) = \pi/24$. $m_H = v\sqrt{\pi/12} = 126.7$~GeV ($+1.1\%$).
See Supplement~S15.
...
$f_\omega = 108\,m_e$ (DERIVED: $C(9,2)(2\ell+1)m_e$, $f_\pi/f_\omega = 5/3$),
...
%----------------------------------------------------------------------
\section{Gravitational waves (new)}
\label{sec:gw}
%----------------------------------------------------------------------

GW dynamical sector derived from the action (CG~\S21, VR~\S31):
$v_\mathrm{GW} = v_\mathrm{EM} = c$ (exact, same null cone).
Scalar radiation kinematically forbidden ($f_\mathrm{thresh} = 3.5 \times 10^{22}$~Hz).
Tensor = standard GR ($\Box h^\mathrm{TT} = -16\pi G T^\mathrm{TT}$).
Predictions: breathing mode polarization (LIGO O5),
$O(\phi^2) \sim 1\%$ merger correction (Cosmic Explorer 2035+).
See Supplement~S17 (CMB) for the Kaleidoscope framework.

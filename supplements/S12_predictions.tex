...
\begin{prediction}
\begin{equation}
  \frac{1}{\alpha_\mathrm{EM}} = \frac{36\pi}{I_2},
\end{equation}
where $I_2 = \int_0^{r_*} e^{2\phi(r)}\,dr$ and
$36 = (2j+1)^2(2l+1)^2 = \binom{9}{2}$.
\end{prediction}

\noindent\textbf{Status:} The formula is structurally exact. The numerical
value depends on $I_2$, computed from the vacuum $\phi$ profile.\\
\textbf{PDG:} $1/\alpha = 137.036$.\\
\textbf{Falsification:} If the formula $36\pi/I_2$ with the locked
parameters $(\phi_0, \mu, r_*)$ gives $1/\alpha$ outside the range
$[135, 139]$, the derivation chain is falsified.
